...
[CreateAssetMenu(fileName = "DecelerateState", menuName = "StateMachines/States/Character/DecelerateState")]
public class DecelerateState : CharacterState
{
    ...
    public DecelerateState() : base(ECharacterState.Decelerating)
    {
    }
    ...
    private float CalculateLedgeDecelerationFactor(PlayerBoundaryElement ground)
    {
        ...
        var rayOrigin = new Vector2(rb.position.x + ((CharacterStateMachine)stateMachine).bodyCollider.bounds.size.x * 1.5f * Mathf.Sign(rb.velocity.x), ((CharacterStateMachine)stateMachine).bodyCollider.bounds.min.y);
