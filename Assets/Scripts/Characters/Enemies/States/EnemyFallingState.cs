...
[CreateAssetMenu(fileName = "FallingState", menuName = "StateMachines/States/Enemy/FallingState")]
public class EnemyFallingState : EnemyState
{
    public EnemyFallingState() : base(EEnemyState.Falling)
    {
    }
    ...
    protected override float? OnPhysicsUpdate()
    {
        ...
        if (((EnemyStateMachine)stateMachine).ground.connected)
        {
            ((EnemyStateMachine)stateMachine).jumpsSinceGrounded = 0;
