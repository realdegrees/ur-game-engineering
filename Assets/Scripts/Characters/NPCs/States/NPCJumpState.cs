...
[CreateAssetMenu(fileName = "JumpState", menuName = "StateMachines/States/NPC/JumpState")]
public class NPCJumpState : NPCState
{
    ...
    {
        ((NPCStateMachine)stateMachine).jumpsSinceGrounded++;
    ...
    protected override float? OnPhysicsUpdate()
    {
        ...
        PlayerBoundaryElement ceiling = ((NPCStateMachine)stateMachine).ceiling;
        RaycastHit2D jumpDirHit = Physics2D.Raycast(((NPCStateMachine)stateMachine).ceilingCheckCollider.transform.position, rb.velocity, .5f, Config.GroundLayer);
